...
WHERE m.store_id IN (@s1,@s2,@s3,@s4,@s5,@s6)
  AND m.sort_order = 1
...
WHERE store_id IN (@s1,@s2,@s3,@s4,@s5,@s6)
...
-- SELECT store_id, sort_order, name, image FROM menu WHERE store_id IN (@s1,@s2,@s3,@s4,@s5,@s6) ORDER BY store_id, sort_order;
-- SELECT store_id, sort_order, image_url FROM store_image WHERE store_id IN (@s1,@s2,@s3,@s4,@s5,@s6) ORDER BY store_id, sort_order;