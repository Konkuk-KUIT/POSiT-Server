...
public class MapController {
    ...
    public ResponseEntity<ApiResponse<MapStoreMarkerResponse>> markers(@Valid MapStoreMarkerQuery query) {
        return ResponseEntity.ok(ApiResponse.success(mapService.getMarkers(query)));
    }

    @Operation(summary = "가게 리스트 조회", description = "화면 아래 가게 리스트 조회")
    @ApiErrorCodes(SwaggerErrorSet.DEFAULT)
    @GetMapping("/stores")
    public ResponseEntity<ApiResponse<MapStoreListResponse>> list(@Valid MapStoreListQuery query) {
    ...
    public ResponseEntity<ApiResponse<MapStoreDetailResponse>> detail(@PathVariable Long storeId) {
        return ResponseEntity.ok(ApiResponse.success(mapService.getDetail(storeId)));
    }
}
